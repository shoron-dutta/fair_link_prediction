import torch
def get_kernel_mat_torch(a, b, r=1, p=2):
...
def get_normalized_kernel_mat_torch(y):
...
def get_mat_renyi_entropy_torch(g, alpha):
...
def get_joint_entropy_torch(g1, g2, alpha):
...
def get_mutual_info_torch(g1, g2, alpha):
  # computes mutual information according to eqn 4
  # g1 and g2 are two normalized kernel matrices of shape [N, N]
  # returns scalar
  return get_mat_renyi_entropy_torch(g1, alpha) + get_mat_renyi_entropy_torch(g2, alpha) - get_joint_entropy_torch(g1, g2, alpha)
